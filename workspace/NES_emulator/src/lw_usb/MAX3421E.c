...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	Status = XSpi_SetSlaveSelect(&SpiInstance, XPAR_SPI_USB_NUM_SS_BITS);

	if(Status != XST_SUCCESS){
		xil_printf("USB DEVICE NOT FOUND");
		return;
	}



	BYTE txBuf[2], rxBuf[2];
	txBuf[0] = reg + 2;
	txBuf[1] = val;
	Status = XSpi_Transfer(&SpiInstance, txBuf, rxBuf, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
    ...
    BYTE sendBUFptr[nbytes+1], receiveBUFptr[nbytes+1];
    sendBUFptr[0] = reg+2;
    for (int i = 0; i < nbytes; i++)
    {
        sendBUFptr[i+1] = data[i];
    }

    Status = XSpi_Transfer(&SpiInstance, sendBUFptr, receiveBUFptr, nbytes+1);
    if(Status != XST_SUCCESS){
           xil_printf("SPI TRANSFER ERROR");
           return NULL;
     }
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//return val
		...
		if (Status != XST_SUCCESS)
	    {
			xil_printf("Slave select not successful chud");
		}

	    //write reg via SPI
		BYTE sendBUFptr[2] = {reg, 0x00};
		BYTE recvBUFptr[2];

	    Status = XSpi_Transfer(&SpiInstance, sendBUFptr, recvBUFptr, 2);
	    if (Status != XST_SUCCESS)
	            {
	                xil_printf("Transfer not successful chud");
	            }


	    //read val via SPI
	    BYTE val = recvBUFptr[1];
	    //read return code from SPI peripheral
	    //if return code != 0 print an error
	    //deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	    XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// Size of buffers is nbytes to read + 1 byte for command
	BYTE txBuf[nbytes+1];
	BYTE rxBuf[nbytes+1];

	// Store command to txbuf
	txBuf[0] = reg;

	Status = XSpi_Transfer(&SpiInstance, txBuf, rxBuf, nbytes+1);
	...
	// Skip 0th element of rxBuf (bad value)
	for(int n=1; n<nbytes+1; n++){
		// data[0] = rxBuf[1] and so on
		data[n-1] = rxBuf[n];
	}
